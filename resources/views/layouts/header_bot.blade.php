
			<div class="horizontal-main bg-dark-transparent clearfix">
				<div class="horizontal-mainwrapper container clearfix">
					<div class="desktoplogo">
						<a href="/"><img src="/assets//images/brand/logo1.png" alt=""></a>
					</div>
					<div class="desktoplogo-1">
						<a href="/"><img src="/assets//images/brand/logo.png" alt=""></a>
					</div>
					<!--Nav-->
					<nav class="horizontalMenu clearfix d-md-flex">
						<ul class="horizontalMenu-list">
							<li aria-haspopup="true"><a href="/" class="active">Home </span></a>
							</li>
							<li aria-haspopup="true"><a href="/about">About Us </a></li>
					 
							<li aria-haspopup="true"><a href="/blog_grid">Blog </a>
								
							
							</li> 
							<li aria-haspopup="true"><a href="/all_ads">Categories <span class="fa fa-caret-down m-0"></span></a>
								<ul class="sub-menu">
									<li aria-haspopup="true"><a href="blog_details">Classifieds Left</a></li>
									<li aria-haspopup="true"><a href="classified-right.html">Classifieds Rights </a></li>
								</ul>

							</li>
						
							<li aria-haspopup="true"><a href="/contact"> Contact Us <span class="wsarrow"></span></a></li>
							
						</ul>
						
					</nav>
					<!--Nav-->
				</div>
			</div>
		</div>